...
int main(void){
    ...
    oldcount = hal_get_time();
    small_ntt((int32_t*)&c_small);
    newcount = hal_get_time();
    printcycles("small NTT:", newcount - oldcount);

    oldcount = hal_get_time();
    small_ntt((int32_t*)&c_small);
    small_point_mul((int16_t*)&c_small_prime, (const int32_t*)&c_small);
    newcount = hal_get_time();
    printcycles("small NTT heavy:", newcount - oldcount);

    oldcount = hal_get_time();
    small_asymmetric_mul((int32_t*)&cs2, (const int32_t*)&c_small, (const int32_t*)&c_small_prime, (const int16_t*)&s2);
    newcount = hal_get_time();
    printcycles("small asymmetric mul:", newcount - oldcount);

    oldcount = hal_get_time();
    small_invntt_tomont((int32_t*)&c_small);
    newcount = hal_get_time();
