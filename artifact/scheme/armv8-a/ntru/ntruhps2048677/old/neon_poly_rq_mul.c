...
static
void karat_neon_evaluate_combine(uint16_t *restrict w, uint16_t *restrict  _poly)
{
    uint16_t *c0 = &_poly[0*SB3_PAD],
             *c1 = &_poly[1*SB3_PAD],
             *c2 = &_poly[2*SB3_PAD],
             *c3 = &_poly[3*SB3_PAD],
             *w0_mem = &w[0*SB3_PAD],
...
static
void karat_neon_interpolate_combine(uint16_t *restrict _poly, uint16_t *restrict w)
{
    ...
    for (uint16_t addr = 0; addr < SB3_RES_PAD; addr += 16)
    {
        ...
        vload_x2(tmp, &_poly[addr + 0*SB3]);
        vadd_x2(r0, tmp, r0);
        vstore_x2(&_poly[addr + 0*SB3], r0);

        vload_x2(tmp, &_poly[addr + 1*SB3]);
        vadd_x2(sum0, tmp, sum0);
        vstore_x2(&_poly[addr + 1*SB3], sum0);

        vload_x2(tmp, &_poly[addr + 2*SB3]);
        vadd_x2(w20, tmp, w20);
        vstore_x2(&_poly[addr + 2*SB3], w20);

        vload_x2(tmp, &_poly[addr + 3*SB3]);
        vadd_x2(sum1, tmp, sum1);
        vstore_x2(&_poly[addr + 3*SB3], sum1);

        vload_x2(tmp, &_poly[addr + 4*SB3]);
        vadd_x2(w21, tmp, w21);
        vstore_x2(&_poly[addr + 4*SB3], w21);

        vload_x2(tmp, &_poly[addr + 5*SB3]);
        vadd_x2(sum2, tmp, sum2);
        vstore_x2(&_poly[addr + 5*SB3], sum2);

        vload_x2(tmp, &_poly[addr + 6*SB3]);
        vadd_x2(r8, tmp, r8);
        vstore_x2(&_poly[addr + 6*SB3], r8);
    }
}

// Ultilize all 32 SIMD registers, Copy
static
void tc4_evaluate_neon_SB1(uint16_t *restrict w[7], uint16_t *restrict _poly)
{
    uint16_t *c0 = _poly,
             *c1 = &_poly[1*SB1],
             *c2 = &_poly[2*SB1],
...
static
void tc4_interpolate_neon_SB1(uint16_t *restrict _poly, uint16_t *restrict w[7])
{
    ...
    for (uint16_t addr = 0; addr < SB1_RES; addr += 8)
    {
        ...
        vload_x1(tmp, &_poly[addr]);
        vadd_x1(r0, tmp, r0);
        vstore_x1(&_poly[addr], r0);

        vload_x1(tmp, &_poly[SB1 + addr]);
        vsub_x1(r1, tmp, r1);
        vstore_x1(&_poly[SB1 + addr], r1);

        vload_x1(tmp, &_poly[2*SB1 + addr]);
        vadd_x1(r2, tmp, r2);
        vstore_x1(&_poly[2*SB1 + addr], r2);

        vload_x1(tmp, &_poly[3*SB1 + addr]);
        vadd_x1(r3, tmp, r3);
        vstore_x1(&_poly[3*SB1 + addr], r3);

        vload_x1(tmp, &_poly[4*SB1 + addr]);
        vsub_x1(r4, tmp, r4);
        vstore_x1(&_poly[4*SB1 + addr], r4);

        vload_x1(tmp, &_poly[5*SB1 + addr]);
        vadd_x1(r5, tmp, r5);
        vstore_x1(&_poly[5*SB1 + addr], r5);

        vload_x1(tmp, &_poly[6*SB1 + addr]);
        vadd_x1(r6, tmp, r6);
        vstore_x1(&_poly[6*SB1 + addr], r6);
    }
}

// Evaluate and copy
static
void tc3_evaluate_neon_SB0(uint16_t *restrict w[5], uint16_t *restrict _poly)
{
    uint16_t *w0_mem = w[0],
             *w1_mem = w[1],
             *w2_mem = w[2],
...
static
void tc3_interpolate_neon_SB0(uint16_t *restrict _poly, uint16_t *w[5])
{
    uint16_t *w0_mem = w[0],
             *w1_mem = w[1],
             *w2_mem = w[2],
    ...
    for (uint16_t addr = 0; addr < SB0_RES; addr+= 16)
    {
        ...
        vload_x2(tmp1, &_poly[addr + 0*SB0]);
        vadd_x2(r0, tmp1, r0);
        vstore_x2(&_poly[addr + 0*SB0], r0);

        vload_x2(tmp2, &_poly[addr + 1*SB0]);
        vadd_x2(c1, tmp2, c1);
        vstore_x2(&_poly[addr + 1*SB0], c1);

        vload_x2(tmp3, &_poly[addr + 2*SB0]);
        vadd_x2(c2, tmp3, c2);
        vstore_x2(&_poly[addr + 2*SB0], c2);

        vload_x2(tmp1, &_poly[addr + 3*SB0]);
        vadd_x2(c3, tmp1, c3);
        vstore_x2(&_poly[addr + 3*SB0], c3);

        vload_x2(tmp2, &_poly[addr + 4*SB0]);
        vadd_x2(r4, tmp2, r4);
        vstore_x2(&_poly[addr + 4*SB0], r4);
    }
}
...
static
void poly_neon_reduction(uint16_t *_poly, uint16_t *tmp)
{
    uint16x8_t mask;
