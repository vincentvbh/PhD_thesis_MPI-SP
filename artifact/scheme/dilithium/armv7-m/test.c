...
static int test_sign(void)
{
    ...
    uint8_t sk[CRYPTO_SECRETKEYBYTES+16];
    uint8_t sm[MLEN + CRYPTO_BYTES+16];
    uint8_t m[MLEN+16];
    ...
    for (i = 0; i < NTESTS; i++) {
        ...
        crypto_sign(sm+8, &smlen, m+8, MLEN, ctx, CTXLEN, sk+8);
#if (!SUPPRESS_OK)
        hal_send_str("crypto_sign DONE.\n");
#endif

        // By relying on m == sm we prevent having to allocate CRYPTO_BYTES twice
        if (crypto_sign_open(sm+8, &mlen, sm+8, smlen, ctx, CTXLEN, pk+8))
        {
