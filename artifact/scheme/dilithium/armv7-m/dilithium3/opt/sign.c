...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
    ...
    for(size_t i = 0; i < K; i++){
        ...
        polyeta_pack(sk + 2 * SEEDBYTES + TRBYTES + L * POLYETA_PACKEDBYTES + i * POLYETA_PACKEDBYTES, &elem0);
        poly_add(&elem1, &elem1, &elem0);

        /* Extract t1 and write public key */
        poly_caddq(&elem1);
        poly_power2round(&elem1, &elem0, &elem1);
        polyt0_pack(sk + 2 * SEEDBYTES + TRBYTES + (L + K) * POLYETA_PACKEDBYTES + i * POLYT0_PACKEDBYTES, &elem0);
...
int crypto_sign_signature(uint8_t *sig,
    ...
    poly h_elem;
    poly cp;
    smallpoly s1_small[L], s2_small[K];
    smallpoly cp_small, cp_small_extended;
    ...
    unpack_sk(rho, tr, key, &t0, &z, &w0, sk);

    for(size_t i = 0; i < L; i++){
        for(size_t j = 0; j < N; j++){
            s1_small[i].coeffs[j] = z.vec[i].coeffs[j];
        }
    }

    for(size_t i = 0; i < K; i++){
        for(size_t j = 0; j < N; j++){
            s2_small[i].coeffs[j] = w0.vec[i].coeffs[j];
        }
    }
    ...
    polyvecl_small_ntt(s1_small);
    polyveck_small_ntt(s2_small);
    polyveck_ntt(&t0);
...
rej:
    ...
    poly_small_ntt_precomp(&cp_small, &cp_small_extended, &cp);
    poly_ntt(&cp);

    pack_sig_setup(sig, sig);

    /* Compute z, reject if it reveals secret */
    polyvecl_small_basemul_invntt(&z, &cp_small, &cp_small_extended, s1_small);
    polyvecl_add(&z, &z, &y);
    polyvecl_reduce(&z);
    ...
    for(size_t i = 0; i < K; i++){
        ...
        * do not reveal secret information */
        poly_small_basemul_invntt(&h_elem, &cp_small, &cp_small_extended, &s2_small[i]);
        ...
        poly_pointwise_montgomery(&h_elem, &cp, &t0.vec[i]);
        ...
        k = pack_sig_h_elem(sig + CTILDEBYTES + L * POLYZ_PACKEDBYTES, &h_elem, i, k);
...
int crypto_sign_verify(const uint8_t *sig,
    ...
    polyvecl_ntt_fast(&z);
    polyvec_matrix_pointwise_montgomery_fast(&w1, mat, &z);
