...
void pack_pk_t1(uint8_t pk[CRYPTO_PUBLICKEYBYTES], const polyveck *t1);
...
void pack_sig_setup(uint8_t sig[CRYPTO_BYTES], const uint8_t c[CTILDEBYTES]);

void pack_sig_z(uint8_t *sig, polyvecl *z);

//
size_t pack_sig_h_elem(uint8_t *sig, poly *h, size_t i, size_t k);
...
void unpack_sk(uint8_t rho[SEEDBYTES], uint8_t tr[TRBYTES], uint8_t key[SEEDBYTES],
                                        polyveck *t0,
                                        polyvecl *s1,
                                        polyveck *s2,
                                        const uint8_t sk[CRYPTO_SECRETKEYBYTES]);

void unpack_sk_rho_tr_key(uint8_t rho[SEEDBYTES], uint8_t tr[TRBYTES], uint8_t key[SEEDBYTES],
                                        const uint8_t sk[CRYPTO_SECRETKEYBYTES]);
...
void unpack_sig_c_z(uint8_t c[CTILDEBYTES], polyvecl *z, const uint8_t sig[CRYPTO_BYTES]);
