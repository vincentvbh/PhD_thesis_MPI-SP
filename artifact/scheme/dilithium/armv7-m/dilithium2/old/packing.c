             ...
             const polyveck* t1)
{
  ...
  for (i = 0; i < K; ++i) polyt1_pack(pk + i * POLYT1_PACKEDBYTES, &t1->vec[i]);
}
...
void unpack_pk_new(uint8_t rho[SEEDBYTES], polyveck* t1, const uint8_t pk[CRYPTO_PUBLICKEYBYTES])
{
  unsigned int i;
  ...
  for (i = 0; i < K; ++i)
    polyt1_unpack_new(&t1->vec[i], pk + i * POLYT1_PACKEDBYTES);
}
...
void pack_sig_z(uint8_t sig[CRYPTO_BYTES], const polyvecl* z)
{
  unsigned int i;
  sig += CTILDEBYTES;
  for (i = 0; i < L; ++i) polyz_pack(sig + i * POLYZ_PACKEDBYTES, &z->vec[i]);
}
...
void pack_sig_h_zero(unsigned char sig[CRYPTO_BYTES], unsigned int* hints_written)
{
  sig += SEEDBYTES;
  sig += L * POLYZ_PACKEDBYTES;
...
int unpack_sig(uint8_t c[CTILDEBYTES], polyvecl* z, polyveck* h, const uint8_t sig[CRYPTO_BYTES])
{
  ...
  for (i = 0; i < L; ++i) polyz_unpack(&z->vec[i], sig + i * POLYZ_PACKEDBYTES);
  sig += L * POLYZ_PACKEDBYTES;
