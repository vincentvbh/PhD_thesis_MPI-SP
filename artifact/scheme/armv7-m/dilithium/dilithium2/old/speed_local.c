...
int main(void){
    ...
    oldcount = hal_get_time();
    polyvecl_pointwise_acc_montgomery_leaktime(&As1.vec[0], &A_row, &s1);
    newcount = hal_get_time();
    ...
    oldcount = hal_get_time();
    polyvecl_ntt_leaktime(&s1);
    for(size_t i = 0; i < K; i++){
        polyvecl_pointwise_acc_montgomery_leaktime(&As1.vec[i], &A_row, &s1);
        poly_reduce(&As1.vec[i]);
        poly_invntt_tomont_leaktime(&As1.vec[i]);
    }
    newcount = hal_get_time();
