...
int crypto_sign_signature(uint8_t *sig,
    ...
    unsigned int n;
    uint16_t nonce = 0;
    int32_t t0_Hom_V[K * 32 * 7 * 4];
    int32_t cp_Hom_M[32 * 7 * 8];
    int32_t buff[32 * 7 * 4];
    ...
    unpack_sk(rho, tr, key, &h, &s1, &s2, sk);
    ...
    for(size_t i = 0; i < K; i++){
        Hom_V(t0_Hom_V + i * 32 * 7 * 4, (int32_t*)&h.vec[i]);
    }
...
rej:
    ...
    memmove(&cp_small, &cp, sizeof(poly));
    poly_ntt_small(&cp_small);
    Hom_M(cp_Hom_M, (int32_t*)&cp);
    ...
    polyveck_pointwise_poly_montgomery_small(&h, &cp_small, &s2);
    polyveck_invntt_tomont_small(&h);
    ...
    for(size_t i = 0; i < K; i++){
        __asm_TMVP_mul_4x4_full(buff, cp_Hom_M, t0_Hom_V + i * 32 * 7 * 4);
        Hom_I((int32_t*)&h.vec[i], buff);
    }
