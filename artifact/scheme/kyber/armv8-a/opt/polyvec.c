...
**************************************************/
extern
void __asm_poly_compress10x8_core(uint8_t *des, const int16_t *src, size_t iter);
extern
void __asm_poly_compress11x8_core(uint8_t *des, const int16_t *src, size_t iter);
void polyvec_compress(uint8_t r[KYBER_POLYVECCOMPRESSEDBYTES], const int16_t a[KYBER_K][KYBER_N]) {
#if KYBER_K == 4
    __asm_poly_compress11x8_core(r, &a[0][0], KYBER_K * KYBER_N / 8);
#elif (KYBER_K == 2 || KYBER_K == 3)
    __asm_poly_compress10x8_core(r, &a[0][0], KYBER_K * KYBER_N / 8);
#else
